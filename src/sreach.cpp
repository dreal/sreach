...
#else
    //"\nUsage: sreach_sq <testfile> <prob_drh-modelfile> <dReach> <k-unfolding_steps_for_dreach_model> <precision>\n\n"
    "\nUsage: sreach_sq <testfile> <prob_drh-modelfile> <k-unfolding_steps_for_dreach_model> <precision>\n\n"
#endif
    "where:\n"
...
//    // prepare some names which may be used later before entering the loop
    ...
    #pragma omp parallel num_threads(maxthreads) shared(result, alldone) firstprivate (dReachcomm, drhname, simresfile, assgn_res, fstrvfile)
    {
